...
- I pay attention to the R ecosystem and read source code frequently.
...
- Style: code needs to breath! We aren't bit packing code and sending it to the moon, so use whitespace. Indent code (to indicate function nesting) because it's easier to read and understand 
...
## How Did I Find My Way? {.smaller}

- **Step zero**: read about package maintenance tools (e.g '[devtools](https://CRAN.R-project.org/package=devtools)', '[lintr]( https://CRAN.R-project.org/package=lintr)', '[styler](https://CRAN.R-project.org/package=styler)', R'[OpenSci recommendations](https://devguide.ropensci.org/index.html') This can be intimidating.
...
- **Step two**: make an easy package fixes or improvementa (e.g. replace `aes_` with `aes`) and learn how to test them. Learn how to use '[testhat](https://testthat.r-lib.org/)' and write a unit test.
- **Step three**: do more complex fixes: I added functionality for specifying  the in response to a feature request. 
...
- Refactoring (some) code is an aspiration, but at this point, I don't dare ("if it ain't broke, don't fix it")
